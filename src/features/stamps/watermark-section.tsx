  ...
  return useMemo<PageOverlayRenderer | null>(() => {
    ...
    return (context) =>
      marked.has(context.page) ? (
        <CentredMark
          context={context}
          spin={form.orientation === 'diagonal' ? 45 : 0}
          mark={{
            text: form.text,
...
export function WatermarkSection({
  ...
  useMarkOverlay(api, OVERLAY_ID, useWatermarkOverlay(form, range.pages, range.error !== null));
  ...
  const apply = (): void => {
    void runner.run('Applying the watermark', async () => {
      ...
      return `Watermarked ${describePageCount(range.pages.length)} with "${form.text}". Save the document to keep it.`;
    });
  };
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <ActionButton
        ...
        disabled={range.error !== null || form.text.trim().length === 0 || runner.busy !== null}
        onClick={apply}
      />
      <Hint>
