...
const bridge: LibrariusBridge = {
  file: {
    ...
    saveTo: (docId, targetPath) => invoke(IPC.file.saveTo, docId, targetPath),
    chooseFolder: () => invoke(IPC.file.chooseFolder),
  ...
  export: {
    ...
    plan: (docId, options) => invoke(IPC.export.plan, docId, options),
