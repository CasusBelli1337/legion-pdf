...
async function pleading500() {
  ...
  for (let p = 1; p <= 500; p++) {
    ...
    page.drawText(`ASHFORD v. RAKIB — DEPOSITION TRANSCRIPT — PAGE MARKER P${String(p).padStart(4, '0')}`, {
      x: 72, y: 730, size: 11, font,
    });
    for (let line = 0; line < 30; line++) {
      page.drawText(`${p}:${line + 1}  ${LOREM.slice(0, 90)}`, { x: 72, y: 700 - line * 20, size: 9, font });
    }
  }
  ...
  manifest['pleading-500.pdf'] = {
    ...
    groundTruth: { firstPageMarker: 'P0001', lastPageMarker: 'P0500', markerFormat: 'PAGE MARKER P####' },
  };
}
...
async function exhibitParts() {
  ...
  for (const [name, count] of Object.entries(sizes)) {
    ...
    for (let p = 1; p <= count; p++) {
      const page = doc.addPage([612, 792]);
      page.drawText(`FILE ${name.toUpperCase()} PAGE ${p} OF ${count}`, { x: 72, y: 396, size: 18, font });
    }
...
async function redactTarget() {
  ...
  for (let p = 0; p < 4; p++) {
    const page = doc.addPage([612, 792]);
    page.drawText(`Financial record page ${p + 1}. Client identifier follows.`, { x: 72, y: 700, size: 12, font });
    page.drawText(secrets[p % secrets.length], { x: 72, y: 650, size: 12, font });
    page.drawText('Non-confidential closing paragraph that must SURVIVE redaction.', { x: 72, y: 600, size: 12, font });
  }
...
async function metadataLaden() {
  ...
  const page = doc.addPage([612, 792]);
  page.drawText('Body text is public. Metadata above must be scrubbed.', { x: 72, y: 396, size: 14, font });
  ...
  manifest['metadata-laden.pdf'] = {
    ...
    groundTruth: { mustVanish: ['Arthur Rothrock', 'Settlement strategy', 'Legion Draft System 9.9', 'privileged'] },
  };
}
...
async function scannedDeposition() {
  ...
  for (let p = 1; p <= 6; p++) {
    ...
    page.drawText('The quick brown fox jumps over the lazy dog 0123456789.', { x: 72, y: 650, size: 14, font });
  }
...
if (files.length !== 8) throw new Error(`Expected 8 fixture files, got ${files.length}: ${files.join(', ')}`);
